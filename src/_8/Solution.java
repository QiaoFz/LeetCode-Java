...
public class Solution {
    public int myAtoi(String str) {
        ...
        if (c == '+') {
            ...
            tmpX = Character.getNumericValue(c);
        }

        for (; i < len; i++) {
            if (Character.isDigit(c = trimStr.charAt(i))) {
                tmpX = tmpX * 10 + c - '0';
                if (sign && tmpX > Integer.MAX_VALUE) {
                    return Integer.MAX_VALUE;
                } else if (!sign && -tmpX < Integer.MIN_VALUE) {
